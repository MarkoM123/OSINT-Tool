...
async def test_assessment_pipeline_completes(db_session: AsyncSession, monkeypatch):
    ...
    async def fake_tech_fingerprint(hosts):
        return [
            {
                "title": "Exposed outdated service",
                "description": "Outdated web server.",
                "evidence": "tech",
                "evidence_source": "technology",
                "confidence": 0.8,
                "severity": "medium",
                "category": "attack_surface",
                "recommendation": "Patch service.",
            }
        ]
        ...
        return [
            {
                ...
                "description": "Default login found.",
                "evidence": "creds",
                "evidence_source": "credentials",
                "confidence": 0.6,
                "severity": "high",
    ...
    monkeypatch.setattr(
        "services.assessment_service.ct_logs.query_ct",
        fake_query_ct,
    )
    monkeypatch.setattr(
        "services.assessment_service.dns.collect_dns_records",
        fake_collect_dns_records,
    )
    monkeypatch.setattr(
        "services.assessment_service.ipinfo.enrich_ips",
        fake_enrich_ips,
    )
    monkeypatch.setattr(
        "services.assessment_service.whois.lookup",
        fake_whois_lookup,
    )
    monkeypatch.setattr(
        "services.assessment_service.email_analyzer.analyze_domain",
        fake_email_analyze,
    )
    monkeypatch.setattr(
        "services.assessment_service.tls_analyzer.analyze_hosts",
        fake_tls_analyze,
    )
    monkeypatch.setattr(
        "services.assessment_service.tech_analyzer.fingerprint_hosts",
        fake_tech_fingerprint,
    )
    ...
    monkeypatch.setattr(
        "services.assessment_service.cred_analyzer.check_credentials",
        fake_cred_check,
    )
    monkeypatch.setattr(
        "services.assessment_service.generate_executive_report",
        fake_generate_executive_report,
    )
    monkeypatch.setattr(
        "services.assessment_service.render_report",
        lambda context: b"%PDF-1.4",
    )
